...
fn compute_next_for_leaf<T: TrainingKey>(num_leaf_models: u64,
    ...
    while idx < num_leaf_models as usize {
        match find_first_above(&first_key_for_leaf, idx as usize) {
            ...
            None => {
                if idx == 0 {
                    break; // partial models have only single model used 
                }
                // let last_next_for_leaf = usize::max(last_key_for_leaf[idx].unwrap().0 + 1, num_keys);
...
impl <T: TrainingKey> LowerBoundCorrection<T> {
    ...
    where F: Fn(T) -> u64 {
        ...
        let next_for_leaf = compute_next_for_leaf(num_leaf_models, data.len(), &first_key_for_leaf,  &last_key_for_leaf);
        ...
        return LowerBoundCorrection {
            ...
            last_nonempty_model_idx: last_nonempty_model_idx,
            first_nonempty_model_idx: first_nonempty_model_idx
