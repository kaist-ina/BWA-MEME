...
fn train_model_big<T: TrainingKey>(model_type: &str,
    data: &RMITrainingData<T>) -> Box<dyn Model> {
    let model: Box<dyn Model> = match model_type {
    "linear" => Box::new(LinearModelBig::new(data)),
    //"robust_linear" => Box::new(RobustLinearModel_big::new(data)),
    //"cubic" => Box::new(CubicSplineModel_big::new(data)),
...
fn train_model<T: TrainingKey>(model_type: &str,
                              data: &RMITrainingData<T>) -> Box<dyn Model> {
    let model: Box<dyn Model> = match model_type {
        "linear_big" => Box::new(LinearModelBig::new(data)),
        ...
        // "pwl" => Box::new(PiecewiselinearModel::new(data,24)),
        "pwl42" => Box::new(PiecewiselinearModel_partial::new(data,26,42)),
...
pub fn train<T: TrainingKey>(data: &RMITrainingData<T>,
    ...
    if model_list.len() == 2 {
        let mut res = two_layer::train_partial_three_layer(&mut data.soft_copy(), &model_list[0],&model_list[1],
                                             &last_model, branch_factor);
        let build_time = SystemTime::now()
            .duration_since(start_time)
            .map(|d| d.as_nanos())
    ...
    // if model_list.len() == 2 {
    //     let mut res = two_layer::train_three_layer(&mut data.soft_copy(), &model_list[0],&model_list[1],
    //                                          &last_model, branch_factor);
    //     let build_time = SystemTime::now()
    //         .duration_since(start_time)
    //         .map(|d| d.as_nanos())
    ...
    // let mut res =  multi_layer::train_multi_layer(data, &model_list, last_model, branch_factor);
    // let build_time = SystemTime::now()
    //         .duration_since(start_time)
    //         .map(|d| d.as_nanos())
    //         .unwrap_or(std::u128::MAX);
